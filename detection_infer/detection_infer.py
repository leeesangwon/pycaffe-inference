...
# def draw_bbox_on_images(input_folder, output_folder, labelmap_file, model_def, model_weights, image_resize, use_center=False):
#     input_list = glob.glob(os.path.join(input_folder, "*.jpg"))
#     input_list += glob.glob(os.path.join(input_folder, "*.JPG"))
#     input_list += glob.glob(os.path.join(input_folder, "*.png"))
#     input_list += glob.glob(os.path.join(input_folder, "*.PNG"))
...
#     assert input_list, "input_folder(%s) doesn't have image file(jpeg or png format)" % input_folder
#
#     h, w = caffe.io.load_image(input_list[0]).shape[0:2]
#     dpi = int(h/10)
...
#         output_image = input_image.replace('target_image', 'result_image/refinedet320_plus_coco')
#
